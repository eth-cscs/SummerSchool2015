...
// ==============================================================================
void serial_write_binary(std::string fname, Field &u, SubDomain &domain, Discretization &options)
{
    FILE *fp = fopen((char*)fname.c_str(), "w" );
    assert(fp != NULL);

    fwrite(u.data(), sizeof(double), domain.N, fp);
    fclose(fp);
}
...
int main(int argc, char* argv[])
{
  ...
  if(domain.rank == 0)
    VisItInitializeSocketAndDumpSimFile(
            ...
            NULL, NULL, strcat(home, "/.visit/simulations/miniapp.sim2"));
#endif
    ...
    if(domain.rank == 0) 
        std::cout << "ENTERING LOOP"
                  << std::endl;
    // main timeloop
    int visitstate, blocking, timestep;
    ...
    if (simulate_one_timestep())
       {
        ...
        str << "output" << std::setw(3) << std::setfill('0') << timestep << ".bin"; // get filename for this time step
    ...
    ////////////////////////////////////////////////////////////////////
    // write individual ( serial) final solutions to raw data files
    ////////////////////////////////////////////////////////////////////
    std::stringstream myFilename, str;

    myFilename << "output." << std::setw(3) << std::setfill('0') << (timestep-1) << "." << std::setw(3) << std::setfill('0') << domain.rank << ".bin";
    serial_write_binary(myFilename.str(), x_old, domain, options);
    ...
    ////////////////////////////////////////////////////////////////////
        ...
        str << "output." << std::setw(3) << std::setfill('0') << (timestep-1) << ".bin"; // get filename for this time step
        write_binary(str.str(), x_old, domain, options); // write binary solution to file

        std::stringstream str2;
        str2 << "output." << std::setw(3) << std::setfill('0') << (timestep-1) << ".bov";

        std::ofstream fid1(str2.str().c_str());
        fid1 << "TIME: " << (timestep-1) << std::endl;
        fid1 << "DATA_FILE: " << str.str() << std::endl;
    ...
    if( domain.rank==0 ) {
        ...
        std::ofstream fid0("combine_serial.bov");
        fid0 << "# BOV version: 1.0" << std::endl;
        fid0 << "# I/O serial, recombining files together" << std::endl;
        fid0 << "TIME: " << (timestep-1) << std::endl;
        fid0 << "DATA_FILE: output." << std::setw(3) << std::setfill('0') << (timestep-1) << ".%03d.bin" << std::endl;
        fid0 << "DATA_SIZE: " << options.nx << " " << options.ny << " 1" << std::endl;
        fid0 << "DATA_BRICKLETS: " << domain.nx << " " << domain.ny << " 1" << std::endl;
        fid0 << "DATA_FORMAT: DOUBLE" << std::endl;
        fid0 << "VARIABLE: phi" << std::endl;
        fid0 << "DATA_ENDIAN: LITTLE" << std::endl;
        fid0 << "CENTERING: nodal" << std::endl;
        fid0 << "BRICK_ORIGIN: 0.0 0.0 0.0" << std::endl;
        fid0 << "BRICK_SIZE: 1.0 " << (options.ny-1)*options.dx << " 1.0" << std::endl;
        ...
        fid2 << "      <Topology TopologyType=\"3DCORECTMESH\" Dimensions=\"1 " << options.ny << " " << options.nx << "\"/>" << std::endl;
        ...
        fid2 << "      <Attribute Name=\"phi\" Active=\"1\" AttributeType=\"Scalar\" Center=\"Node\">" << std::endl;
        fid2 << "          <DataItem Dimensions=\"1 " << options.ny << " " << options.nx << "\" NumberType=\"Float\" Precision=\"8\"" << std::endl;
        fid2 << "                    Format=\"Binary\">" << str.str() << std::endl;
        fid2 << "<!--" << std::endl;
        fid2 << "                    Format=\"HDF5\">output.h5:/data/phi" << std::endl;     
        fid2 << "-->" << std::endl;
        fid2 << "          </DataItem>" << std::endl;
