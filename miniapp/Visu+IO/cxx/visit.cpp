...
SimGetMesh(int procid, const char *name, void *cbdata)
{
  ...
  if(strcmp(name, "mesh") == 0)
    ...
    if(VisIt_RectilinearMesh_alloc(&h) != VISIT_ERROR)
      ...
      minRealIndex[0] = domain.startx-1;// default case for a domain not touching the walls
      minRealIndex[1] = domain.starty-1;// default case for a domain not touching the walls
      minRealIndex[2] = 0;  
      maxRealIndex[0] = domain.endx-1; // options.nx;// default case for a domain not touching the walls
      maxRealIndex[1] = domain.endy-1; //options.ny;// default case for a domain not touching the walls
      maxRealIndex[2] = 0;
if (procid == 4)
{
std::cerr << "domain.startx = " << domain.startx << "\n"
     << "domain.starty = " << domain.starty << "\n"
     << "domain.endx = " << domain.endx << "\n"
     << "domain.endy = " << domain.endy << "\n";
}
...
SimGetDomainBoundaries(const char *name, void *cbdata)
{
    ...
    if(VisIt_DomainBoundaries_alloc(&h) != VISIT_ERROR)
    {
        ...
        VisIt_DomainBoundaries_set_numDomains(h, domain.size);
        ...
        for(dz = 0; dz < domain.size; ++dz)
          {
      ext[0] = domain.startx-1;// default case for a domain not touching the walls
      ext[1] = domain.starty-1;// default case for a domain not touching the walls
      ext[2] = 0;  
      ext[3] = domain.endx-1; // options.nx;// default case for a domain not touching the walls
      ext[4] = domain.endy-1; //options.ny;// default case for a domain not touching the walls
      ext[5] = 0;
if (dz == 33)
{
std::cerr << "domain.startx = " << domain.startx << "\n"
     ...
     << "domain.endy = " << domain.endy << "\n";
          ...
          VisIt_DomainBoundaries_set_rectIndices(h, dz, ext);
          }
    }
...
VisItSetGetDomainBoundaries(SimGetDomainBoundaries, (void*)NULL);
}
#endif
